...
import { ArrowLeftStartOnRectangleIcon, Cog6ToothIcon, MoonIcon, SunIcon, UserIcon } from "@heroicons/react/16/solid";
            ...
            <div className="md:hidden flex items-center">
                <Drawer>
                    <DrawerTrigger asChild>
                        {auth ? <Avatar>
                            <AvatarImage src={avatar(auth.me.user.avatar ?? undefined, auth.me.user.id)} />
                            <AvatarFallback>{auth.me.user.username.substring(0, 2).toUpperCase()}</AvatarFallback>
                        </Avatar> : <Button size={"icon"}><Cog6ToothIcon className="w-4 h-4" /></Button>}
                    </DrawerTrigger>
                    <DrawerContent>
                        <DrawerHeader>
                        ...
                        <div className="flex flex-col gap-2 p-4">
                            {auth && <><h3>Auth</h3><div className="flex flex-col gap-1">
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                >
                                    ...
                                    <MoonIcon className="w-4 mr-2 flex dark:hidden" /></Button>
                            </div>
                            <h3>Color theme</h3>
                            <div className="flex flex-col gap-1">
                                <Button variant={"secondary"}
                                    className="w-full justify-between" onClick={(e) => setColor(e.currentTarget.value)} value={"main"}>Default</Button>
                                <Button variant={"secondary"}
                                    className="w-full justify-between" onClick={(e) => setColor(e.currentTarget.value)} value="pine">Pine</Button>
                                <Button variant={"secondary"}
                                    className="w-full justify-between" onClick={(e) => setColor(e.currentTarget.value)} value="coral">Coral</Button>
                            </div>
                        </div>
                    </DrawerContent>
                </Drawer>
