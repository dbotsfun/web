...
export default function BotDeveloper({ id }: BotDeveloperProps) {
	const [resetApiKey, apiKeyResult] = useResetApiKeyMutation();

	useEffect(() => {
		if (apiKeyResult.called && apiKeyResult.data?.resetAPIKey)
			toast.success(
				"API Key has been reset, once you change the tab you will not be able to see key again",
			);
	}, [apiKeyResult]);
	return (
		<Card>
			...
			<CardContent>
				{apiKeyResult.loading ? (
					...
					<Input
						...
						aria-readonly
						className="w-full"
						value={apiKeyResult.data?.resetAPIKey}
					/>
				) : (
					<Credenza>
						<CredenzaTrigger asChild>
							<Button><ArrowPathIcon className="w-5 h-5 mr-2" />Reset API Key</Button>
						</CredenzaTrigger>
						<CredenzaContent>
							<CredenzaHeader>
							...
							<CredenzaFooter>
								...
								<Button
									disabled={apiKeyResult.loading || apiKeyResult.called}
									onClick={() =>
										resetApiKey({
											variables: {
												input: {
