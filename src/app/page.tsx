...
import { ArrowPathRoundedSquareIcon, ArrowTrendingUpIcon, ClockIcon, FireIcon, MagnifyingGlassIcon } from "@heroicons/react/20/solid";
            ...
            <div className="flex flex-col gap-5 mt-5">
              <div className="relative" ref={searchRef}>
                <div className="relative w-full flex gap-2 items-center">
                  <Input value={query ?? ""} onFocus={() => setFocus(true)} onKeyDown={(e) => {
                    if (e.key === "Enter" && e.currentTarget.value.length) setHistory([...history!, e.currentTarget.value])
                  }} onChange={(e) => setQuery(e.currentTarget.value)} placeholder="Start typing to see popover" className="h-12 duration-150 px-4 focus-visible:!ring-background focus-visible:!ring-offset-primary" />
                  ...
                  <div className="text-left break-all w-full flex flex-col space-y-2 py-4">
                    <div>
                      <div className="flex justify-between items-center">
                        <p className="text-muted-foreground px-4 text-xs mb-2">Search History</p>
                      ...
                      <div className="px-4 py-2">
                        ...
                        </Link>) : "No search history"}
                      </div>
                    </div>
                    <Separator className="bg-accent" />
                    <div className="space-y-2 py-4">
                      <p className="text-muted-foreground px-4 text-sm mb-2">Search Results</p>
                    </div>
                  </div>
                </div>}
              </div>
